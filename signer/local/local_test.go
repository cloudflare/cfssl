...
func TestExtensionSign(t *testing.T) {
	...
	// By default, disallowed extensions should be ignored.
	request := signer.SignRequest{
		Request: string(csrPEM),
	...
	certPEM, err := s.Sign(request)
	if err != nil {
		t.Fatalf("Failed to sign a cert from a CSR with an unrecognized extension")
	}
	cert, err := helpers.ParseCertificatePEM(certPEM)
	if err != nil {
		t.Fatalf("Failed to parse a cert generated by a CSR with an unrecognized extension: %s", err)
	}
	if cert.ExtraExtensions != nil {
		t.Fatalf("Expected signed cert to have no ExtraExtensions because none were whitelisted, found %#v",
